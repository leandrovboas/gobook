...
func (h *BookHandlers) SearchBooksByName(w http.ResponseWriter, r *http.Request) {
	name := r.PathValue("name")

	book, err := h.service.SearchBooksByName(name)

	if err != nil {
		http.Error(w, fmt.Sprintf("Book %s not found", name), http.StatusNotFound)
	}
...
func (h *BookHandlers) SimulateReading(w http.ResponseWriter, r *http.Request) {
	...
	book := h.service.SimulateMultipleReadings(bookIDs, 2*time.Second)
